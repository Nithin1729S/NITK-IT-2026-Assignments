...
def compare_ego_centralities(graphs, ego_names):
    ...
    for ego in ego_names:
        ...
        # Get centrality for the ego node
...
def analyze_spectral_centralities(graphs, ego_names, output_dir='outputs'):
    ...
    for ego in ego_names:
        ...
        # Top influencers by each metric
        ...
        # Visualize rank order comparison
...
def analyze_structural_comparison(graphs, ego_names, output_dir='outputs'):
    ...
    for ego in ego_names:
        ...
        # Calculate centralization index (Freeman's)
        ...
        # Hierarchy coefficient (variance in centrality)
    ...
    # Calculate hierarchy score (higher = more hierarchical)
    ...
    # Calculate decentralization score (higher = more decentralized)
...
def visualize_structural_comparison(df_struct, graphs, ego_names, output_dir):
    ...
    # 1. Radar chart for structural dimensions
    ...
    # 2. Hierarchy vs Decentralization scatter
    ax2 = plt.subplot(222)
    for idx, ego in enumerate(ego_names):
        row = df_struct[df_struct['Ego']==ego].iloc[0]
    ...
    # 4. Density and clustering comparison
    ax4 = plt.subplot(224)
    x = np.arange(len(ego_names))
    ax4.bar(x - width/2, df_struct['Density'], width, 
           label='Density', alpha=0.8, color='#2ecc71')
    ax4.bar(x + width/2, df_struct['Clustering'], width, 
           label='Clustering', alpha=0.8, color='#f39c12')
    ...
    # 5. Individual network structure visualization
    fig, axes = plt.subplots(2, 2, figsize=(16, 14))
    axes = axes.flatten()
...
def analyze_networks():
    ...
    # NEW ANALYSES
    # 1. Compare local centralities across egos
    ego_centrality_df = compare_ego_centralities(graphs, ego_names)
    
    # 2. Spectral centrality analysis
    spectral_data = analyze_spectral_centralities(graphs, ego_names)
    
    # 3. Structural comparison (hierarchy vs decentralized)

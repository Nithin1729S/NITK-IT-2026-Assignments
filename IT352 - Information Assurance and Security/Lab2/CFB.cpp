...
int main(){
    ...
    for(int i=0;i<inputBinaryString.length();i+=FEEDBACK_SIZE){
       string afterEncWithKeyAndShiftReg=encrypt(shiftRegister,keyBinaryString).substr(0,FEEDBACK_SIZE);
       string cipherTextBlock=xor_(afterEncWithKeyAndShiftReg,inputBinaryString.substr(i,FEEDBACK_SIZE));
       encryptedString+=cipherTextBlock;
       shiftRegister=shiftRegister.substr(FEEDBACK_SIZE) + cipherTextBlock;
    }
    ...
    for(int i=0;i<encryptedString.length();i+=FEEDBACK_SIZE){
        string afterEncWithKeyAndShiftReg=decrypt(shiftRegister,keyBinaryString).substr(0,FEEDBACK_SIZE);
        string plainTextBlock=xor_(afterEncWithKeyAndShiftReg,encryptedString.substr(i,FEEDBACK_SIZE));
        decryptedBinaryString+=plainTextBlock;
        shiftRegister=shiftRegister.substr(FEEDBACK_SIZE) + encryptedString.substr(i,FEEDBACK_SIZE);
    }
